...
stateGameWindow::stateGameWindow(cApp*  pParent)
{
    m_pGame = pParent;
}

void stateGameWindow::initialize()
{
   // load up whatever resources I need (background, sounds, etc)
    hge->System_Log("INIT Main");
}

void stateGameWindow::shutdown()
{
...
void stateGameWindow::captureinput(float dt)
{
    ...
    if (!m_pGame->GetGUIManager()->GUICharPress(hge->Input_GetChar()))
	{
		...
		switch (tmpKey)
		{
			...
			case HGEK_5:
				break;
			case HGEK_6:
				//m_GameObjMangr->SetAmbientColor(0xFFFFFFFF);
				break;
			case HGEK_7:
//				m_GameObjMangr->SetAmbientColor(200,200,250);
				break;
			case HGEK_8:
				//m_GameObjMangr->SetAmbientColor(0xFF89B9D0);
				break;
			case HGEK_9:
				//m_GameObjMangr->SetAmbientColor(50,50,200);
				break;
			case HGEK_LEFT:
				//m_ChatWindow->LostFocus();
				break;
			case HGEK_RIGHT:
				//m_ChatWindow->SetFocus();
				break;
			case HGEK_UP:
				//m_ChatWindow->HideCaption();
				break;
			case HGEK_DOWN:
				//m_ChatWindow->ShowCaption();
				break;
			case HGEK_A:
				//m_TargetSystem->SetMode(TMATTACK);
				//mainmap->UpdateTarget();
				break;
			case HGEK_D:
	//			m_ChatWindow->ToggleVisible();
				break;
			case HGEK_W:
				//m_TargetSystem->SetMode(TMWALK);
				break;
			case HGEK_S:
				break;
			case HGEK_Q:
